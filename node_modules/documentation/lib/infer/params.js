...
function inferParams(comment) {
  ...
  if (t.isClassDeclaration(path) && !(comment.constructorComment && comment.constructorComment.hideconstructor)) {
...
function mapTags(tags) {
  return new Map(tags.map(function (tag) {
    return [tag.name, tag];
  }));
}
    ...
    case 'ObjectPattern':
      {
        ...
        if (prefix === '') {
          ...
          return {
            ...
            properties: _lodash2.default.flatMap(param.properties, function (prop) {
    ...
    case 'ArrayPattern':
      {
        ...
        if (prefix === '') {
          return {
            ...
            // indices
            properties: _lodash2.default.flatMap(param.elements, function (element, idx) {
...
function mergeTopNodes(inferred, explicit) {
  ...
  var inferredNames = new Set(inferred.map(function (tag) {
    return tag.name;
  }));
  ...
  return {
    errors: errors,
    mergedParams: inferred.map(function (inferredTag) {
...
function mergeNodes(inferred, explicit) {
  var intersection = _lodash2.default.intersectionBy(inferred, explicit, function (tag) {
    return tag.name;
  });
  var explicitOnly = _lodash2.default.differenceBy(explicit, inferred, function (tag) {
    return tag.name;
  });
  var inferredOnly = _lodash2.default.differenceBy(inferred, explicit, function (tag) {
    return tag.name;
  });
  var mapExplicit = mapTags(explicit);

  return intersection.map(function (inferredTag) {
    var explicitTag = mapExplicit.get(inferredTag.name);
    return explicitTag ? combineTags(inferredTag, explicitTag) : inferredTag;
...
function combineTags(inferredTag, explicitTag) {
  ...
  var hasProperties = inferredTag.properties && inferredTag.properties.length || explicitTag.properties && explicitTag.properties.length;
  ...
  } : {}, { type: type }, defaultValue ? { default: defaultValue } : {});
}
